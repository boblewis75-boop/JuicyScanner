...
def scan_options(
    ...
    for c in contracts:
        ...
        iv_rank    = _estimate_iv_rank(c["volatility"])
        ...
        prob = calculate_likelihood(
            option_type      = c["putCall"],
            underlying_price = underlying_price,
            strike           = c["strikePrice"],
            premium          = premium,
            iv               = c["volatility"] / 100,  # convert % to decimal
            dte              = c["daysToExpiration"],
            delta            = abs(c["delta"]),
            target_pct       = abs(target_pct),
            tech_score       = tech_bull_score if c["putCall"] == "CALL" else (100 - tech_bull_score),
        )
